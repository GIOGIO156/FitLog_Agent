...
Chat 草稿回复采用一个稳定 envelope：面向用户的解释、估算依据和确认提示放在 `message.text`，可保存的结构化 Food Draft / Workout Draft 放在 `draft`，Edge Function 校验后只把原生 artifact 卡片展示给用户，不把 provider 原始 JSON 当普通 assistant 消息渲染。Add Food 的 AI 食物分析仍走专用 `ai-food-photo-analyze` 路径，要求返回纯结构化 Food Draft JSON，不需要聊天式解释。
