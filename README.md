...
进入真实会话后，消息列表从顶部 history/account/provider 控件下方开始，并按实测输入框、底部导航和安全区统一避让底部遮挡。发送新问题时，新用户气泡会在完成布局后锚到顶部可读区域附近，assistant loading 保持可见；发送中的活动轮次填充不会暴露成可滚动空白，最终回复出现后不再强制二次滚动。AI 背景始终是单个全屏动效层，进入阅读态后整体变慢，而不是只让顶部或底部遮挡区移动。
...
- Add Food 把图片 AI 分析作为第一入口；用户拍照或一次选择最多三张相册图片，可点按缩略图切换放大预览，补充说明后由服务端千问多模态返回 Food Draft。
- 继续支持手动录入、外部 AI JSON 粘贴 fallback、预览编辑、复制到指定日期和删除。
- AI 不确定肉类、分量、是否已吃完或烹饪方式时，应先追问。
- 图片 AI Food Draft 会进入现有 Food Preview 编辑页；Chat Food Draft 先显示 artifact 卡片，用户点击后才用 snapshot 打开 Food Preview。
...
Once a real conversation exists, the message list starts below the top history/account/provider controls and uses one shared bottom obstruction based on the measured composer, bottom navigation, and system safe area. Sending a new question anchors that user bubble near the top of the readable area after it has a real layout position while the assistant loading bubble remains visible; send-time active-turn fill is not exposed as scrollable blank space, and the final reply does not force a second scroll. The AI background stays one full-screen animated layer and slows down as a whole in reading mode instead of moving only in top or bottom exposed areas.
...
Except for the Add Food photo-recognition shortcut, Agent workflows start from AI Chat. Chat-generated Food Draft / Workout Draft artifacts first show a structured summary and review button; tapping the button rebuilds Food Preview or the workout editor draft from the stored snapshot, and the user still has to save from the corresponding editor.

### Core Features

Food logging:

- Add Food makes Photo AI Analysis the first entry; the user can take or choose up to three images, tap thumbnails to switch the enlarged preview, add an optional note, and receive a Qwen multimodal Food Draft from the server.
...
- Photo AI Food Drafts open in the existing Food Preview editor; Chat Food Drafts first appear as artifact cards and open Food Preview only after the user taps review.
