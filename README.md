...
注意：当前源码仍主要是复制来的 FitLog Local 实现；账号、订阅、Cloud Profile、AI Gateway、App 内 LLM、RAG 和 AI Chat 属于 Agent V1 目标设计，需在后续工程阶段实现。
...
AI 页面位于底部导航正中间，建议导航结构为：

```text
Home | Food | AI | Workout | Profile
```

AI 页面是一个简单 Chat，而不是 quick chips 工作台。页面包含全屏动态背景、中心状态文案、底部输入框、右上角账号/订阅入口和左侧可折叠历史栏。未登录、未联网或未订阅时，AI 页面切换为灰色不可用状态；如果用户已经输入了一半的 prompt，可以继续编辑，但不能发送。
...
- Agent V1 目标设计包含后端 AI Gateway、账号、订阅、云端 Profile 和远程模型调用
...
Note: the current source code is still mostly the copied FitLog Local implementation. Account login, subscription, Cloud Profile, AI Gateway, app-internal LLM calls, RAG, and AI Chat are Agent V1 target designs that need later engineering implementation.
...
The AI page is a simple Chat surface, not a quick-chip workspace. It includes a full-screen animated background, center status copy, bottom composer, top-right account/subscription entry, and left collapsible history sidebar. When the user is signed out, offline, or unsubscribed, the page switches to a grayscale disabled state. A half-written prompt remains editable, but sending is disabled.
...
- Agent V1 target design includes backend AI Gateway, account, subscription, Cloud Profile, and remote model calls
