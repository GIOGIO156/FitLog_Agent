...
Add Food 把 AI 食物分析放在第一入口。页面删除重复的解释副标题，并把与 AI Chat 相互独立的 ChatGPT/千问选择器直接放在标题下方、图片区域之前。用户可以只输入文字描述，也可以同时添加最多三张相机/相册图片。大预览框是唯一的图片来源入口：未选图时显示“点击添加食物图片”；选图后显示当前图片，右下角只保留一个小型 `+`。点击任一状态都会打开底部弹窗，其中拍照和相册两个操作并排显示。预览框下方紧接三个固定尺寸缩略位，因此只添加一两张图片时不会放大缩略图，也不会改变页面高度；缩略图栏与预览保持一个紧凑图片组，并在食物描述前留出更明确的 16 px 分组间距。相册选择只请求剩余名额；三张已满后再次选择会替换当前放大的图片而不是添加第四张，任何超过请求上限的 picker 结果都会被拒绝，不能静默截断。

当前发布已配置 Qwen；选择 ChatGPT 时显示正常生命周期的“当前模型不可用”错误，选择器短暂响应后自动滑回千问，保留文字和图片，不发送 request，也不改变 AI Chat 选择。模型选择器直接显示，不再嵌套在额外卡片中。分析按钮作为悬浮控件覆盖在页面上，不占用整行 footer；从按钮中线到屏幕物理底部只绘制与按钮等宽的实体背景遮挡，既阻止滚动内容从按钮下方透出，也不在按钮上方形成大块纯色区域。列表始终预留相同的底部空间。聚焦食物描述后，分析按钮、遮挡层、列表、TextField 和 FocusNode 全部保持挂载。实时键盘 inset 只负责固定尺寸描述框的纵向运动，并将描述框作为一个刚体平移以保持键盘间距；不存在逐帧 reveal、补偿滚动、动态 padding 切换或键盘高度缓存。分析按钮、遮挡层和列表底部留白使用稳定的底部 `viewPadding`，在键盘运动期间始终保持键盘关闭时的屏幕 geometry。系统键盘只负责覆盖和揭示按钮，不再触发透明度、平移、缩放或安全区恢复位移；键盘完全关闭前继续禁止按钮输入。小型本地 picker-recovery marker 允许 Android activity 重建后恢复分析内容，而不是返回空页面。原分析页仍挂载时，它独占 picker 结果，Root 不再打开第二份页面；只有页面确实丢失时才执行一次恢复。
