...
- 说明型 guide 使用 root modal 阅读层；固定雾化 scrim 禁用底部导航，正文在需要时独立滚动，并在导航 footprint 上方保留可读空间。黑橙主题下 guide 面板使用实体 elevated surface，页面文字不能透过弹窗显示。
...
- 通知只在 App 离开前台后出现；点击时先取消通知，再只恢复同一草稿的一个编辑页。回到 App、保存、放弃或删除全部动作也会取消通知。
...
Add Food 把 AI 食物分析放在第一入口。页面删除重复的解释副标题，并把与 AI Chat 相互独立的 ChatGPT/千问选择器直接放在标题下方、图片区域之前。用户可以只输入文字描述，也可以同时添加最多三张相机/相册图片。大预览框是唯一的图片来源入口：未选图时显示“点击添加食物图片”；选图后显示当前图片，右下角只保留一个小型 `+`。点击任一状态都会打开底部弹窗，其中拍照和相册使用横向并排的大卡片；AI Chat 复用同一卡片布局，并显示自己的标题和已选数量摘要。预览框下方紧接三个固定尺寸缩略位，因此只添加一两张图片时不会放大缩略图，也不会改变页面高度；缩略图栏与预览保持一个紧凑图片组，并在食物描述前留出更明确的 16 px 分组间距。相册选择只请求剩余名额；三张已满后再次选择会替换当前放大的图片而不是添加第四张，任何超过请求上限的 picker 结果都会被拒绝，不能静默截断。

当前发布已配置 Qwen；选择 ChatGPT 时显示正常生命周期的“当前模型不可用”错误，选择器短暂响应后自动滑回千问，保留文字和图片，不发送 request，也不改变 AI Chat 选择。模型选择器直接显示，不再嵌套在额外卡片中。分析按钮作为悬浮控件覆盖在页面上，不占用整行 footer；从按钮中线到屏幕物理底部只绘制与按钮等宽的实体背景遮挡，既阻止滚动内容从按钮下方透出，也不在按钮上方形成大块纯色区域。列表始终预留相同的底部空间。聚焦食物描述后，分析按钮、遮挡层、列表、TextField 和 FocusNode 全部保持挂载。实时键盘 inset 只负责固定尺寸描述框的纵向运动，并将描述框作为一个刚体平移以保持键盘间距；不存在逐帧 reveal、补偿滚动、动态 padding 切换或键盘高度缓存。运动期间，标题、provider selector 和图片区域逐渐变暗并停止接收输入，描述框保持在前景继续支持光标和选区调整；点击或竖向拖动灰色背景只收起键盘，不会打开图片选择。分析按钮、遮挡层和列表底部留白使用稳定的底部 `viewPadding`，在键盘运动期间始终保持键盘关闭时的屏幕 geometry。系统键盘只负责覆盖和揭示按钮，不再触发透明度、平移、缩放或安全区恢复位移；键盘完全关闭前继续禁止按钮输入。小型本地 picker-recovery marker 允许 Android activity 重建后恢复分析内容，而不是返回空页面。原分析页仍挂载时，它独占 picker 结果，Root 不再打开第二份页面；只有页面确实丢失时才执行一次恢复。
...
Paste AI Result 在静止状态保持固定且不可滚动。键盘运动期间始终复用同一个固定尺寸 JSON editor，页面关闭 Scaffold 自动 resize，让实时键盘 inset 成为纵向运动的唯一 owner。Inset 将整个 editor 作为一个刚体平移以保持键盘间距；静止“解析”操作边界附近使用一个很小的连续交接区，消除之前的硬启动点，同时不缓存键盘高度也不增加 animation controller。设置卡片跟随键盘展开过程逐步淡出，同时保留 layout footprint 和状态，因此不会在单帧内突然消失，也不会带动 editor 改变位置；固定的“解析”操作保持挂载、禁止输入并由键盘自然覆盖。Editor 右上角的放大操作使用低强调、中性色的四角框图标，不再使用强调色实心控件。它先清除焦点并等待真实键盘 inset 归零，再打开只包含大型 JSON editor 的 root modal；缩小操作沿用相同的安静视觉样式。弹窗使用实体 elevated panel 和固定雾化背景缝隙，不自动重新聚焦。临时 controller 保留文字和选区，关闭后把编辑值写回原页面，弹窗内不复制“解析”或 Prompt 操作。
...
- Composer 接受文字和最多三张 JPEG、PNG 或 WebP 图片；附件操作打开与 AI 食物分析相同的横向拍照/相册双大卡片，并显示 Chat 专属标题和已选数量摘要。相册可以一次填满剩余名额。当前文字和图片 turn 通过 Qwen 路由。选择未配置 ChatGPT 时显示相同的短暂错误，选择器使用与底部导航一致的滑动动画自动回到千问，保留 composer 和附件且不发送 request。状态灯仍保持订阅/设备/Gateway readiness 的真实状态；自动回到千问只是 UI 恢复，不会暗中发送请求。具体模型名和 provider key 只保留在服务端。
...
- 历史会话行内重命名在键盘改变侧栏高度时仍归属于当前 session；字段只使用 scrollable 的正常焦点 reveal，不追加延迟补偿，因此较低行在 inset 变化时仍保留 editor 和未提交名称。
...
如果 Android 在这类新建训练编辑期间重建进程，最近 30 分钟内更新的草稿会连同原字段自动回到 Add Workout；进程仍存活时保持当前编辑页，不重复导航。更早的草稿仍可通过恢复条手动继续，但不会强制打开。明确返回、丢弃和成功保存都会停止自动重开。这是启动时的 route restoration，不是 30 分钟计时器，也不会要求系统保留进程。
...
用户开始正式保存训练后，生命周期 autosave 停止，当前项进入只读状态，FitLog 只确认同一个稳定保存 mutation；保存过程中切到其它 App 不会再创建一份可编辑草稿。如果响应丢失，Workout 显示“正在确认保存”的条目，而不是把它当成未保存训练；打开该条目会用同一 mutation 手动重试，原进程恢复时只查询已记录结果。如果进程被强制结束，下次启动只会确认或放弃旧 mutation，不自动重发训练。确认成功后移除该条目，确认放弃后恢复正常编辑，状态服务不可用时继续保持锁定。
...
- Auth landing、登录和注册始终使用同一棵稳定布局树，键盘关闭时画布不可滚动。键盘打开后只给同一画布增加临时滚动范围，由唯一的 inset owner 立即跟随键盘移动，只在需要时移动焦点字段，并在键盘上方保留 14 px 间距，不再为下一字段或主要操作按钮额外预留空间。键盘 Next 明确按“登录邮箱→密码”和“注册邮箱→验证码→密码→确认密码”推进，每次只执行一次短促焦点切换定位，不再由计时器或键盘稳定后的校正启动第二段位移。键盘关闭后恢复零偏移和滚动锁定。
...
当前身体资料包括年龄、身高、体重、性别、体脂率和腰围。当前身高、体重、体脂和腰围按一位小数保存；“保存更改”会立即把输入框同步为规范保存值并一次性清除草稿。历史体重、体脂和腰围使用独立身体记录流程。
...
“导出与数据”中的“清空本地数据”必须经过二次确认。该操作只清空当前设备 SQLite 的业务表，包括可重建的 food/workout/body/summary cache，以及本地训练草稿、自定义动作、校准与复盘状态和兼容 Profile 数据。它不退出账号，不删除 Supabase session、SharedPreferences 中的语言/主题等偏好、已导出文件、Cloud Profile、云端正式 records、AI chat history 或云端 AI logs。

登录状态下清空后，页面仍以 Cloud Profile 和 Cloud Records 为权威；后续刷新可以把云端仍存在的 Profile、身体、饮食、训练和 summary 数据重新写入本地 cache。训练草稿、自定义动作、校准或复盘状态等本地独有数据没有云端副本，清空后无法恢复。因此该操作不能描述为账号删除、云端删除或仅删除可重建 cache。精确 SQLite 表范围见 [Database.md](Database.md)，权威与回填规则见 [CloudLocalDataBoundary.md](CloudLocalDataBoundary.md)。
