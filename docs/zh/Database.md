...
- `final_answer_json` 可以保存轻量 `ai_chat_artifacts.v1` snapshot，用于经过校验的返回 artifact，例如 `food_draft.v1`。该 snapshot 只用于用户点击确认按钮后重建 Preview 页面，不是正式记录，也不是后台草稿队列。
