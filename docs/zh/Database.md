...
| 存储 | 用途 | 权威与生命周期 |
| --- | --- | --- |
| SQLite / `sqflite` | 本地兼容 profile/cache、校准、策略复盘、自定义动作、训练草稿、训练提交 ledger、账号绑定 confirmed read model 和选中日期 `daily_summary_cache`。 | Schema v18。登录后正式记录在这里不具权威；confirmed cache 可重建且有界。 |
| Supabase Cloud Records | `body_metric_logs`、`food_records`/`food_items`、`workout_sessions`/`workout_sets`、`workout_plan_commits` 和 `daily_summaries`。 | 登录后正式记录的权威来源；cloud-backed repository 协调写入和本地 read-model 更新。 |
...
| AI 文档索引 | 面向 Document RAG 的版本化 active-build 稳定文档 chunks 与 embeddings。 | Supabase Postgres lexical/vector candidate retrieval，只允许 service role staging、activation、search 与 rollback。 |
| In-memory providers | 选中日期、刷新版本、App services、语言状态和运行时摘要。 | 只用于短暂运行期协调。 |

当前本地数据库名：`fitlog_local.db`。

当前本地 SQLite schema version：`18`。
...
| 16 | 删除旧 `edit_record` 训练草稿，使草稿恢复只适用于用户手动或 AI 创建的新训练记录。 |
| 17 | 为 `workout_record_drafts` 增加训练保存状态字段，并新增只含 hash 的 `workout_plan_commits` ledger。幂等 upgrade 与 open-time repair 能修复中间 v17 测试构建，无需清除用户数据。 |
| 18 | 为本地训练 mutation ledger 增加进程启动恢复所用的 `abandoned` 终态；幂等 upgrade 与 open-time repair 会保留已有 committed rows。 |
...
- `total_load` 直接把 `input_weight_kg` 作为计算负重；`per_side_load` 保存 `calculation_load_kg = input_weight_kg * 2`。
- `bodyweight_added` 和 `assistance_load` 保留输入的外部/辅助重量；热量服务结合保存时体重快照计算，不覆盖原始字段。
- `total_reps` 直接把 `input_reps` 作为计算次数；`per_side_reps` 保存 `calculation_reps = input_reps * 2`。
- `duration_seconds` 保留 `input_duration_seconds`；其有边界的计算等价值单独保存，不改变界面时长。
...
用途：一个由用户手动创建或 AI 交接的当前未保存新建训练编辑器状态。
...
| `kind` | 当前写入只使用 `new_record`；旧 `edit_record` 行由 v16 migration 删除。 |
| `source_plan_id`, `source_session_id` | 为旧数据保留的 nullable 兼容字段；当前新建记录草稿留空。 |
| `date`, `record_name`, `notes` | 草稿可见 metadata。 |
| `payload_json` | 序列化编辑器快照。 |
| `save_state` | `editing`、`committing` 或 `commit_unknown`；只有 `editing` 允许生命周期 autosave。 |
| `save_mutation_id` | 在正式结果确认前持续复用的稳定 idempotency key。 |
| `target_plan_id` | 正式提交使用的稳定 plan id。 |
| `save_payload_hash` | Canonical commit payload 的 SHA-256；ledger 不重复保存正式 payload。 |
| `save_started_at` | 重试复用的稳定正式保存时间。 |
| `save_body_weight_kg` | 为该保存 payload 冻结的计算体重。 |
| `created_at`, `updated_at` | ISO 时间戳。 |
...
- 草稿不进入 Home totals。
- 草稿不是正式训练历史。
- 编辑已保存训练只使用页面内状态，绝不写入该表。
- 显式保存会先验证编辑器状态，再写入正式训练表。
- 草稿里的力量组条目可以在 `payload_json` 内携带 draft-only 的 `completed_at` 时间戳，用于 Android 训练进行中通知判断最近一次勾选完成组；该字段仍不进入正式 `workout_sets` SQLite schema。
- `committing` 和 `commit_unknown` row 是锁定的确认状态，不是第二份可编辑训练；它们不会自动恢复 editor，也不会创建“训练进行中”通知。

### `workout_plan_commits`

用途：SQLite 兼容保存路径的本地幂等与恢复 ledger。

| Field | Meaning |
| --- | --- |
| `account_scope`, `mutation_id` | Composite primary key，防止同一账号/本地 mutation 被应用两次。 |
| `operation` | `create`、`replace_plan` 或 `replace_session`。 |
| `target_plan_id` | 包含已提交本地 sessions 的 plan。 |
| `payload_hash` | Canonical request 的 SHA-256 identity；本表不复制训练 payload 或 notes。 |
| `status` | Atomic terminal value `committed` 或 `abandoned`。 |
| `committed_at` | ISO 终态解析时间；对 `committed` row 表示 commit 时间。 |

本地 workout rows 与 committed ledger row 在同一 SQLite transaction 内写入。新进程恢复时要么找到已提交 mutation，要么先写入 abandoned tombstone，再把保留项恢复为普通可编辑草稿。
...
`AppDatabase.clearAllLocalData()` 在一个 SQLite transaction 中删除当前所有业务表 rows：`food_items`、`food_records`、`workout_sets`、`workout_sessions`、`workout_record_drafts`、`workout_plan_commits`、`custom_exercises`、`user_weight_logs`、`calorie_calibration_state`、`diet_adjustment_reviews`、`user_profile` 和 `daily_summary_cache`。数据库文件、schema 和 migration version 保持不变。

该方法不访问 Supabase，不清除 Auth session、SharedPreferences、内存 controller 状态或本地导出文件。云端正式 Profile/records/summaries 仍然存在，并可通过正常读取路径重新建立 SQLite cache；本地训练草稿、自定义动作、校准与复盘状态没有云端恢复保证。产品语义和 UI 提示由 [Product.md](Product.md) 与 [AppGuide.md](AppGuide.md) 维护，权威与回填不变量由 [CloudLocalDataBoundary.md](CloudLocalDataBoundary.md) 维护。
...
用途：正式训练 plan 保存的账号作用域幂等与结果查询。

`(account_id, mutation_id)` primary key 保存 operation、target plan、canonical payload hash、status（`pending`、`committed` 或 `abandoned`）、result session IDs 和 timestamps，但不保存训练 payload。`commit_workout_plan_v1` 校验 active device，在同一 PostgreSQL transaction 内按需 soft delete 被替换目标、插入全部新 sessions/sets，并把 ledger 标记为 committed。相同 hash 的重试返回已记录 sessions；不同 hash 抛出 `idempotency_conflict`。`get_workout_plan_commit_v1` 只用于前台只读确认；新进程恢复使用与 commit 共用 mutation advisory lock 的 `abandon_workout_plan_commit_v1`，要么返回已提交结果，要么记录 abandoned tombstone 并拒绝延迟到达的旧 commit。Authenticated client 的 direct table access 被撤销，只能调用受守卫的 RPC。
...
- `final_answer_json` 可以保存轻量 `ai_chat_artifacts.v2` snapshot，例如合法 `food_draft.v2` 或 `workout_draft.v3`，同时保存解析后的 `target_date`、日期解析来源、`ai_chat_evidence.v1` 或概括 retrieved context 的 `evidence` object。Artifact snapshot 只在 review 后重建 Preview；evidence 只用于只读显示/debug context。两者都不是正式记录或后台草稿队列。History reader 继续兼容 v1/v2 artifact：legacy draft 没有自身日期时使用其中保存的 selected date。
...
用途：有限、可恢复 Chat clarification 的权威控制状态。

字段包括 account/session 和 origin message IDs、可选 parent clarification、`ai_chat_clarification.v2`、kind、allowlisted options、missing dimensions、attachment policy、progress signature、state、attempt count、resolution request/option/result、expiry 与 timestamps。State 取值为 `pending`、`resolving`、`resolved`、`superseded`、`cancelled` 或 `expired`。

规则：

- Account/session RLS 只允许客户端读取自己的 active state；service-owned RPC 负责 compare-and-transition mutation。
- Partial unique index 保证每个 session 最多一条 `pending` 或 `resolving` row。
- Claim 校验 account、session、clarification ID、option、expiry 与 request ID。同一个已完成 request ID 重放其 accepted terminal result；并发或过时冲突 request 不会重复调用 provider。
- Claim 失败可安全回到 `pending`；重复且无进展的 intent/missing-business signature 被拒绝。Intent selection 与业务缺字段 follow-up 各自最多一步。
- Options/missing dimensions 只是紧凑控制 metadata。原始消息文字仍在正常 message row；本表不保存图片 bytes/base64 或 runtime attachment identifier。
- `attachment_policy` 只说明 Flutter 能否重新绑定当前运行期像素。历史重载、进程重启、清空本地数据、退出或切换账号后，不可用像素进入 `resend_required`，不能从云端重建。
- 该 cloud-only additive migration 不改变 SQLite `AppDatabase.dbVersion`，也不改变本地 `workout_record_drafts` 的权威地位。
...
- Additive migration `202607150001_rag_latency_breakdown.sql` 增加有界的 `ai_latency_breakdown.v1` object，分别记录 Edge runtime age、环境/鉴权/请求/设备校验、规划、Context 构建、query normalization、embedding、hybrid RPC、rerank、rewrite planner、Provider 生成/校验/纠错和持久化。缺失或不适用阶段使用 `null` 或 `not_requested`；该 object 不保存 raw message、向量、文档摘录、图片、Provider output、secret、业务记录或 chain-of-thought。
- Additive migration `202607150002_ai_chat_turn_rag_workflows.sql` 让 `record_ai_chat_turn` RPC validation 与 table contract 对 `workout_logging`、`general_chat` 和 `safety_boundary` 的支持保持一致；既有 workflow 与 service-role-only 执行边界不变。
- Chat 使用 `prompt_version = chat_orchestration_v2` 与 `schema_version = ai_chat_response.v3`；Add Food 使用 `workflow_type = food_logging` 与 `schema_version = food_draft.v2`。Additive workflow constraints 在保留 legacy values 的同时，也接受服务端规划的 `workout_logging`、`general_chat` 与 `safety_boundary`。
- Additive migrations `202607190001_ai_chat_clarification_state.sql` 与 `202607190002_ai_chat_orchestration_observability.sql` 增加服务端权威 clarification 状态/RPC，以及紧凑 decision、clarification、attachment、failure、write-guard 和 shadow-mismatch 分类；这些字段不保存 request-time pixels、完整 provider output 或 prompt。后续 migration `202607190003_ai_chat_clarification_digest_search_path.sql` 为 security-definer turn RPC 明确加入 Supabase `extensions` schema，使 `pgcrypto.digest` 在新建和升级项目中都能计算 progress signature。
...
- 该服务端 operational table 保存紧凑 Gateway summary。Additive observability fields 覆盖 surface/capability/provider-policy 版本、Task Plan source 与 approved/rejected Context types、经过审查的 canonical concept IDs、corpus/build/embedding/reranker 版本、branch counts、coverage/retry/issues、有界 latency/context-size metrics、grounding/language/Food semantic status、fact/conflict counts 和 final action。Add Food 还保存 input kind、selected date、note presence、image count，以及可用时的 mime type/compressed byte length。Authenticated client 没有直接 read policy。
- 这些字段不得重复保存完整问题、custom exercise 名称、raw Context/history rows、原图/base64、provider invalid output、secret 或 chain-of-thought。
...
- Migration `202607080001_phase5_document_rag_index.sql` 创建兼容 legacy 的基础表；additive migration `202607130001_rag_foundation_document_hybrid.sql` 在不删除 lexical rows 的前提下增加版本化 corpus build 和 1536 维 embedding metadata。Migration `202607150003_rag_hybrid_indexed_candidates.sql` 增加 generated `search_tsv`、GIN index 与有界 indexed-candidate retrieval；`202607150004_rag_parallel_candidate_fusion.sql` 增加 lexical-candidate 与最终 v3 fusion RPC，同时保留兼容 functions。
- `document_corpus_builds` 保存 staging/active/superseded build state、预期 source/chunk 数、manifest/generator/term 版本和可选 embedding model/dimension；未完成 build 不会与上一 active build 混合暴露。
- Legacy `search_document_chunks`、foundation `search_document_chunks_hybrid` 与有界 v2 保留用于兼容和 fail-closed rollback。生产先通过 service-role-only `search_document_chunk_lexical_candidates_v1` 获取 indexed term/FTS/trigram candidate IDs，同时执行 query embedding；随后由 `search_document_chunks_hybrid_v3` 强制 active corpus、language、authority 与 status filters，计算全局 lexical/vector ranks，并返回最多 30 个 fused candidates 供 Edge rerank。Staging/activation RPC 校验 source/chunk parity；要求 embedding 的 activation 还校验 vector count。
- `supabase/seed_phase5_document_chunks.sql` 和可审查 corpus-build JSON 由 `tool/phase5_document_rag/build_document_chunks.mjs` 从 canonical manifest 生成。Qwen `text-embedding-v4` build/sync/verify 是幂等的，固定使用 1536 维，并在 cloud activation 前校验 content、chunker、terms、model、dimension 和 input hash。
...
- `build_exercise_history_summary` 是只允许 service role 的有界正式 workout session/set 聚合；它按 verified account、最多四个 exercise keys、有界日期和 session limit 过滤，不返回 notes 或完整 set rows。
