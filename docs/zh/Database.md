...
| 云端数据库 | Supabase Auth 账号身份、订阅 entitlement rows、Cloud Profile、AI chat sessions/messages、AI request logs、compact debug summaries 和 document chunks。 | Phase 2 migration 已新增 `subscriptions` 和 `cloud_profiles`；Phase 4 Step 1 新增受保护的 AI chat/log/debug 表；Phase 4 Steps 2-4 新增 Gateway Edge Function、服务端 chat-turn/session RPC、真实 text provider metadata 和 App 侧云端 history 读取；Phase 5 新增 `document_chunks` 和 `search_document_chunks`。 |
| AI 文档索引 | 面向 Document RAG 的可检索 App 文档块。 | 已用 Supabase Postgres 实现关键词/全文/trigram 检索，并且只允许 service-role 写入。 |
...
- `README.md` 或已索引的 `docs/en/*` / `docs/zh/*` 改动不会自动更新云端 rows。如果改动会影响 App Logic Q&A，应重新生成 `supabase/seed_phase5_document_chunks.sql` 并在 Supabase 执行。
...
- 重新生成的 seed SQL 会在插入当前 chunks 前删除已索引来源路径对应的 managed document corpus，避免重命名或删除的标题继续可检索。
...
| `document_context` | `document_chunks` 通过 `search_document_chunks`。 | 包含来源文档路径、section id、heading path、chunk 位置、status、score、context prefix、可选 context note 和 excerpt。 |
