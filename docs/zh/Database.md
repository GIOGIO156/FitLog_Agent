...
当前复制来的源码使用 FitLog Local 的 SQLite schema。Agent V1 可以新增云端存储，用于账号、订阅、Cloud Profile、AI sessions、AI messages、request metadata 和紧凑 debug summaries。V1 不默认完整云同步 food、workout 或 weight 历史。

## 存储总览

| 存储 | 用途 | 当前状态 |
| --- | --- | --- |
| SQLite / `sqflite` | 本地饮食、训练、体重、profile 基线、校准、策略复盘、自定义动作、训练草稿。 | Local 基线已实现。 |
| SharedPreferences | UI 语言偏好和轻量 app 偏好。 | Local 基线已实现。 |
| 本地文件 | App documents directory 中的 XLSX 和 CSV ZIP 导出。 | Local 基线已实现。 |
| 云端数据库 | 账号、订阅、Cloud Profile、AI chats、AI request logs、AI 最终回答、紧凑 debug summaries。 | Agent V1 计划。 |
...
以下是 Agent V1 的服务端存储概念。命名是设计名，实际后端可调整。
