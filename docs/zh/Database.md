...
| 存储 | 用途 | 权威与生命周期 |
| --- | --- | --- |
| SQLite / `sqflite` | 本地兼容 profile/cache、校准、策略复盘、自定义动作、训练草稿、账号绑定 confirmed read model 和选中日期 `daily_summary_cache`。 | Schema v16。登录后正式记录在这里不具权威；confirmed cache 可重建且有界。 |
...
`AppDatabase.clearAllLocalData()` 在一个 SQLite transaction 中删除当前所有业务表 rows：`food_items`、`food_records`、`workout_sets`、`workout_sessions`、`workout_record_drafts`、`custom_exercises`、`user_weight_logs`、`calorie_calibration_state`、`diet_adjustment_reviews`、`user_profile` 和 `daily_summary_cache`。数据库文件、schema 和 migration version 保持不变。
