...
- 在明确的 Phase 7 云同步迁移前，本地 food/workout/weight 历史视为设备数据集。
...
| AI | Agent 主入口。 | Phase 2 已实现居中 tab、不可用状态 AI shell、可编辑输入框、模型选择器、账号/订阅状态 sheet、订阅/Profile 可用性 gating 和本机记录授权开关；AI Gateway 接入前仍不能发送。 | AI Gateway 调用、云端 chat history、饮食草稿、用餐决策、周复盘和 App 规则问答。 |
...
- 非 AI 页面使用实体主题色导航 pill 遮住滚动内容；AI 页面使用玻璃态导航 pill，让动效背景仍然可见。
...
- 输入框可编辑，但发送按钮在 Phase 3 AI Gateway 前禁用。
- 模型选择器显示 `ChatGPT` 和 `千问`，仅作为 UI 占位。
- 账号/订阅入口打开 Phase 2 状态 sheet，包含退出登录和本机记录摘要授权开关。中心状态文案优先读取已保存的 Cloud Profile 昵称，再回退到 auth display name。
...
Phase 2 可以显示账号、订阅和 Profile 都就绪的视觉状态，但消息发送仍锁到 Phase 3 Gateway 和 chat-history contract 接入之后。
...
- 登录和注册失败会保留当前表单，并通过底部 snackbar 显示可读提示。
...
- Profile 页面底部提供明确的账号卡片用于退出登录。退出登录会清空 auth session 和本地 singleton Profile 缓存，但不删除本机 food/workout/weight 记录。
- 离线时禁止保存 Profile。
...
训练、饮食、体重记录 V1 默认仍在本地。只有用户请求需要时，才汇总后供 AI 使用。在未来明确的云同步阶段前，这些记录不会静默归属到新登录账号。

Profile 的身体资料区显示六项：年龄、身高、体重、性别、体脂和腰围。身体趋势卡片位于身体资料下方，可以在本地账号作用域内查看体重、体脂或腰围在 7/14/21/28 天窗口中的折线。指标和天数选择位于卡片底部，周期变化和记录数量位于折线图上方；真实记录点按当前窗口内的真实日期间隔从左向右延伸；当前周期记录不足等状态直接显示在折线图区内；点按真实记录点会在图内显示该点数值。
...
- Phase 2 account controller 与 Auth、订阅状态、Cloud Profile、本机记录授权 repository。
...
- 持久化 Supabase 登录态恢复、AI 账号/订阅状态 sheet、带紧凑模糊浮层状态刷新和内部兑换码 entitlement 的 Profile 顶部“订阅”入口、本机记录摘要授权开关、Profile 底部退出登录账号卡片，以及退出登录/切换账号时清空输入草稿。
