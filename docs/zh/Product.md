...
- Android 会把包含任意已选动作的未保存训练草稿镜像成系统“训练进行中”通知。它只是 draft-resume surface，不是后台任务或新正式记录。
  ...
  - 点击恢复同一个本地 draft；保存、舍弃或删除全部动作后取消通知。
...
- AI message list 把 composer 当成真实遮挡物，但不使用全宽 footer plate。Viewport 从顶部 action row 下方开始、hard clip 旧消息，并保留短 bottom fade。键盘关闭时 viewport 截止在 composer 上方；键盘打开时延伸到浮动实心 composer 后方直到键盘顶部，用列表 padding 保护最后气泡。过渡期间 composer offset 取键盘 inset 与正常导航静止避让中的较大值。
...
- 账号/订阅入口打开当前状态 sheet，包含退出登录和用户记录摘要授权开关。该授权控制 routed 用餐决策和复盘 workflow 是否可以使用云端记录摘要；关闭时 Gateway 仍可使用同会话、Cloud Profile 和文档上下文，但会把记录摘要维度报告为缺失。中心状态文案优先读取已保存的 Cloud Profile 昵称，再回退到 auth display name。
...
- AI 页面调用 `ai-chat-route` Edge Function 发送文本 turn 和最多三张图片的千问多模态 turn，请求语言按当前用户消息推断，并通过服务端 RPC 持久化通过校验的 user/assistant 文本消息。
...
- AI 页面可以通过千问发送最多三张 JPEG/PNG/WebP 图片。Chat 图片附件打开系统相机/相册前会保存很小的本地恢复标记，让 Android activity 重建后可以恢复输入文字和取回的图片附件。Food Draft 响应会在用户点击确认后打开 Food Preview，Workout Draft 响应会在用户点击确认后打开现有训练编辑草稿；二者都携带通过校验的目标日期，在 review 前展示，并通过正常、跟随主题的日历控件提供修改，仍需用户在编辑页保存后才写正式记录。
...
正式训练保存开始后会冻结生命周期 autosave。云端确认成功后，系统按顺序执行最终草稿删除，因此保存过程中切到后台不会留下重复的过期草稿；保存失败则保留可编辑草稿。
...
Profile 身体资料区显示年龄、身高、体重、性别、体脂和腰围。这些都是普通 Profile draft 字段，没有卡片级保存键，只随完整 Cloud Profile 通过底部“保存更改”提交。
