...
| Food Log | 正式饮食记录管理。 | 手动饮食录入、外部 AI JSON 粘贴、复制到日期、编辑、删除，以及 Add Food 图片 AI 分析确认保存后的 `ai_photo` 记录。 | 登录后正式记录 cloud-first 写入；后续 Chat 草稿流程也必须保留同样的用户确认边界。 |
| Add Food | 饮食创建流程。 | 图片 AI 分析是第一入口；手动录入和外部 AI JSON 粘贴仍保留。 | 未来如加入超过三张 Chat 图片或更完整的编辑页内草稿调整，必须先有明确隐私和确认设计。 |
| AI | Agent 主入口。 | 当前 AI 页面已实现居中 tab、可用性 gating 的 Chat 页面、可编辑输入框、可选择/复制的消息气泡、最多三张千问图片附件、本机持久化模型选择器、只表达可用性的状态 pill、账号/订阅状态 sheet、订阅/Profile 可用性 gating、用户记录摘要授权开关、紧凑同会话 context、Gateway client、服务端 OpenAI/Qwen provider 路由、Chat Food Draft 和 Workout Draft artifact 卡片，以及支持新建/切换/inline 重命名/确认删除的云端 chat history。 | 基于 RAG 的用餐决策、周复盘、App 规则问答和更完整的草稿编辑。 |
...
- 全屏 AI 动效背景；空白首页使用清晰可见的彩色流动，键盘输入、等待回复和阅读消息时使用极慢、低幅度动效，让页面不会完全静止，也不抢夺文字阅读注意力。
...
- 发送时输入框立即清空，用户消息先显示为 pending 气泡，等待期间显示 assistant loading 气泡；如果发送失败，会恢复刚才尝试发送的草稿。
- assistant 消息通过维护中的 GitHub-flavored Markdown 渲染器按 App 样式展示，文本可选择，不加载远程图片，也不执行链接动作。用户消息保持可选择的普通文本。每条文本消息气泡还提供复制动作，复制原始消息文本。
...
| 状态 | UI 行为 |
| --- | --- |
| 已登录、联网、已订阅 | 彩色 AI 背景；允许发送；状态指示为绿色。 |
| 等待 assistant 回复 | 背景使用极慢低幅度动效；assistant loading 气泡显示处理状态。 |
...
Add Food 现在把“图片 AI 分析”作为第一入口。用户可以拍照或从相册一次选择最多三张图片，点击缩略图切换上方放大预览，补充可选说明，并发送到专用 `ai-food-photo-analyze` Gateway 路径。服务端调用千问多模态能力并返回经过 schema 校验的 Food Draft，不直接写入正式记录。草稿会进入现有 Food Preview 编辑页；只有用户点击保存后，才以 `ai_photo` 来源写入 `food_records` / `food_items`。
...
当前已实现的草稿界面包括 Add Food 图片分析后的 Food Preview、用户点击确认后重建 Food Preview 的 Chat Food Draft artifact 卡片，以及用户点击确认后重建现有训练编辑草稿的 Chat Workout Draft artifact 卡片。未来更完整的草稿界面也应与对应记录编辑页 UI 保持一致，并支持：
...
- Chat 发送链路，包含最多三张千问图片附件、紧凑同会话 context、用户 pending 气泡、assistant loading 反馈、语义状态指示、可选择/复制的消息气泡、维护中的 assistant Markdown 渲染、Food Draft 进入 Food Preview 的确认路径，以及 Workout Draft 进入训练编辑草稿的确认路径。
