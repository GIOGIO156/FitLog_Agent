...
- Agent 版正式记录功能登录前置。
- Phase 3 Cloud Records Foundation 后，body/food/workout 正式记录以云端为 source of truth。
...
| --- | --- | --- | --- |
| Home | 选中日期仪表盘。 | 本地日汇总、饮食上下文、宏量/kcal 展示、紧凑饮食/训练卡。 | Phase 3 后读取 cloud-backed daily summaries，可出现低打扰 AI 入口提示，但应路由到 AI 页面。 |
| Food Log | 正式饮食记录管理。 | 手动饮食录入、外部 AI JSON 粘贴、复制到日期、编辑、删除。 | Phase 3 后正式记录写入云端；接收 AI Chat 或 Add Food 拍照识别确认后的 Food Draft。 |
...
| Workout | 正式训练记录管理。 | 本地训练记录、自定义动作、训练草稿编辑器、热量启发式计算。 | Phase 3 后正式记录写入云端；V1 AI 可解释或复盘训练上下文，但不静默修改训练记录。 |
...
Phase 3 后，训练、饮食和身体指标记录以云端为正式来源。本地 SQLite 只是 partial cache；默认预取最近 30 天和必要 summaries，用户查看更早历史时按日期或月份从云端按需加载。AI 使用记录时读取云端 summary/context builder，不把本地 cache 当权威来源。

Profile 的身体资料区显示当前 Profile 六项：年龄、身高、体重、性别、体脂和腰围。身体资料卡提供日历/新增身体记录入口；用户选择日期后打开身体记录 sheet，顶部只显示具体日期，sheet 只编辑体重、体脂和腰围三项。过去日期补记不会静默修改当前 Cloud Profile；如需把某条历史记录设为当前身体资料，必须显式确认。身体趋势卡片位于身体资料下方，只读展示体重、体脂或腰围在 7/14/21/28 天窗口中的折线，不承担记录入口。指标和天数选择位于卡片底部，周期变化和记录数量位于折线图上方；真实记录点按当前窗口内的真实日期间隔从左向右延伸；当前周期记录不足等状态直接显示在折线图区内；点按真实记录点会在图内显示该点数值。
...
AI 请求只上传最小必要上下文。用户业务数据上下文应优先使用云端摘要，而不是原始行。cache 清理只删除可重建本地缓存，不删除云端正式数据。
