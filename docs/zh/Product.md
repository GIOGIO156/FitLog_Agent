...
所有 AI Chat provider 回复都使用统一经过校验的 envelope：`message.text` 承载友好解释和确认提示，`draft` 承载结构化 Food Draft 或 Workout Draft。OpenAI 使用 strict Structured Outputs，Qwen 使用 JSON Mode；Gateway 再执行共享精确校验与最多一次受限纠错。只有最终校验通过后 App 才展示解释/artifact，绝不把 raw provider JSON 当作 assistant Markdown。Food Draft item totals 会在审查前归一化。专用 Add Food 路径使用更窄的版本化 envelope 和同一 Food Draft 校验器，再进入 Food Preview。
