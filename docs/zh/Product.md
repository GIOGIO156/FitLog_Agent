...
- assistant 消息支持基础 Markdown 渲染，包括段落、标题、加粗、有序/无序列表、行内代码和代码块；用户消息保持普通文本。
...
- Chat 发送链路，包含最多三张千问图片附件、紧凑同会话 context、用户 pending 气泡、assistant loading 反馈、语义状态指示、局部 assistant Markdown 渲染、Food Draft 进入 Food Preview 的确认路径，以及 Workout Draft 进入训练编辑草稿的确认路径。
