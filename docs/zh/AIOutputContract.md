...
本文不定义检索输入、文档索引或上下文组装；这些内容属于 [RAGDesign.md](RAGDesign.md)。本文不定义 Flutter 与 Gateway 之间的 HTTP transport 字段；这些内容继续由 [../API_CONTRACT_DRAFT.md](../API_CONTRACT_DRAFT.md) 维护。阶段状态和剩余 Provider 启用 Gate 属于 Roadmap；已完成的 rollout 证据保留在 Phase 5 工程历史中。
...
普通 AI Chat 使用 provider-neutral 的 `chat_decision.v2` contract，并固定优先级：消费匹配的 typed clarification reply；执行 safety 与 fixed-entry constraints；只接受少量中英对称的高置信结构；其余请求进入一次有界文字或多模态 planner。Decision 选择 capability、planned workflow、允许/选中的 output family、requested/approved/rejected Context、clarification 状态、附件策略、source、confidence 和紧凑 reason codes。兼容的 `task_plan.v1` 从该 decision 派生，不再与它竞争。Flutter 不能提交或覆盖 decision/`expected_output`。
...
公开 response 还携带 `ai_chat_clarification.v2`：opaque clarification ID、kind、可见 question、allowlisted option IDs 及其 resulting output family、missing dimensions、attachment policy、attempt 和 expiry。Reply 携带 clarification ID、option ID 和稳定 client request ID。云端状态机在任何新意图推断前幂等 claim reply，每个 session 只允许一个 active clarification，并终止重复 no-progress、expired 或 conflict transition。消费 option 时恢复的是原任务及其必要的授权 Context，例如训练规则问题同时恢复文档证据与动作定义，而不只是恢复 output 枚举。自然语言 option alias 只在该 pending state 内有效，不能成为全局 intent keyword。
...
- `provider_unavailable`：当前部署没有配置所选 provider/model
...
- `planner_unavailable`：有界 decision planner 无法访问或未完成
- `planner_output_invalid`：planner 返回非法 decision contract
- `clarification_conflict`：reply 已过时、已被 claim，或不匹配当前 session state
- `clarification_expired`：pending clarification 已不能消费
- `attachment_unavailable`：任务需要的 request-time pixels 已不存在
...
- Chat decision 与 clarification-state schemas
...
当前 AI Chat 使用公开 response schema `ai_chat_response.v3`、prompt version `chat_orchestration_v2`、decision schema `chat_decision.v2` 和 clarification schema `ai_chat_clarification.v2`。Client 继续 additive 读取旧消息与 artifacts，但不会把旧 free-text prompt 重新解释为 typed clarification。
...
- decision version/source/reason、selected capability、clarification ID/state/attempt、attachment policy，以及为 rollout 证据保留但在 legacy 退场后通常为空的历史 shadow-mismatch category
...
- 文字和图片路径
- 中文和英文
- fixed/deterministic/model selection、typed clarification consumption/idempotency、当前图片 planning、普通回答、Food Draft、Workout Draft、假成功声明、refusal、truncation、malformed JSON、错类型、缺字段、额外字段、被动存储说明与不支持的写入声明
