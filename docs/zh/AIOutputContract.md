...
| AI Chat，Qwen 文字与图片 | 所有回复均使用 Qwen 非思考 JSON Mode 与 `provider_gateway_envelope.v1`。 | 解析严格 envelope，按服务端解析出的 expected output 校验；期待 draft 时不得降级为 prose success。 |
...
Gateway 拥有一个内部 expected-output 决策：

| Expected output | 合法结果 |
| --- | --- |
| `text` | 合法 Chat envelope，包含用户可见 `message.text`、`draft = null`，且没有不支持的写入声明。 |
| `food_draft` | 合法 Chat envelope，包含 `food_draft.v1`，或一次有界 clarification。 |
| `workout_draft` | 合法 Chat envelope，包含 `workout_draft.v1`，或一次有界 clarification。 |

Clarification 是 draft expectation 的合法结果：`needs_clarification = true`、questions 非空、`draft = null`。Safety blocked 是 provider call 前由 Gateway 确定性生成的 response，不是 expected provider-output 值。Routing 与 output validation 相互独立：Routing 决定允许返回什么；validation 证明实际 payload 满足这个决定。

Resolver 由服务端负责：read-only route 解析为 `text`；routed `food_logging` workflow 解析为 `food_draft`；明确的饮食/训练记录意图解析为对应 draft；紧凑同会话 clarification reply 可以延续上一种 draft family。Flutter 不能提交或覆盖 `expected_output`。
...
专用 Add Food endpoint 可以保留更窄的公开 response envelope，但必须与 AI Chat 共用同一个 canonical `food_draft.v1` 定义和同一条校验/归一化 pipeline。
...
5. **Workflow validation**：实际 payload 必须满足 routed expected output。
6. **Domain validation and normalization**：应用 Food item totals 重算、真实日期检查等 FitLog 不变量。
7. **Safety/write validation**：移除或拒绝不支持的写入声明，并阻止 read-only workflow 返回 draft。
8. **Client compatibility validation**：Flutter 可以拒绝无法安全重建编辑器的历史 artifact snapshot。
...
Provider alias 或模型升级不能静默改变 accepted contract。Schema 变化应尽量 additive、包含 fixture coverage，并保持历史 artifact 可读。无法安全重建的旧 history artifact 继续显示为 disabled summary。
...
- 文字和图片路径
- 中文和英文
- 普通回答、clarification、Food Draft、Workout Draft、refusal、truncation、malformed JSON、错类型、缺字段、额外字段、不支持的写入声明
