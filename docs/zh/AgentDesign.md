...
| Photo AI Analysis | Add Food 第一入口。用户选择一到三张拍照/相册图片，可点击缩略图切换放大预览，也可用缩略图角标删除单张图片，并可补充说明；`ai-food-photo-analyze` 调用千问多模态 provider，返回 schema 校验后的 Food Draft 并打开 Food Preview。 | 服务端中介草稿 | `AddFoodPage`, `PhotoFoodAnalysisPage`, `AiFoodPhotoAnalysisClient`, `ai-food-photo-analyze` |
...
| 状态 | 视觉行为 | 产品含义 |
| --- | --- | --- |
| 空闲首页 | 彩色清晰、有可见流动。 | AI 页面可用或已进入，等待用户输入。 |
| 输入、等待或阅读 | 极慢、低幅度流动。 | 用户正在输入、等待回复或阅读消息，聊天内容拥有注意力。 |
| Disabled | 灰色、低动态但仍持续流动。 | 用户离线、未登录或未订阅。输入框仍可编辑，但不能发送。 |

当消息列表变长并可滚动时，背景动效始终是 AI 页面背后的单个全屏层。它在消息层后方变得极慢、低亮度、低饱和，保证可读性，但不应被拆成只有顶部/底部在动的分段遮罩。背景不应完全停止，包括键盘打开时。

等待 provider 回复时，应由 chat UI 表达活跃状态，而不是让背景加速。发送后 composer 立即清空，用户消息进入 pending 气泡；等这个气泡有了真实布局位置后，消息列表把它锚到顶部操作区下方的可读区域附近。为了让 pending 用户气泡和 assistant loading 气泡稳定靠上，可以保留很小的活动轮次尾部填充，但它不能表现成一大段可滚动空白，用户手势也不能把 pending/loading 这一组完全滑出视野。assistant loading 气泡显示到最终 assistant 消息从云端 history 重新加载完成。assistant 回复出现后不再强制二次滚动。
...
- 消息列表从顶部 history/account/provider 控件下方开始，尽量扩大正文阅读空间。
- 消息列表底部需要预留足够空间，覆盖实测 composer 高度、bottom navigation、系统安全区和底部 veil。
...
- composer 和消息列表不能各自独立计算底部间距；它们应共用同一个底部遮挡高度，包括键盘、实测 composer 高度、bottom navigation、系统安全区和 veil。
...
4. 当前已实现的 Chat 图片草稿路径先展示 Chat artifact 卡片，只有用户点击确认按钮后才打开现有 Food Preview 编辑页。
5. 用户可以在保存前编辑草稿字段。
...
已实现的图片路径都是受限 workflow，不是 RAG。Add Food 把一到三张压缩后的 JPEG/PNG/WebP 图片和可选补充说明发送到 `ai-food-photo-analyze`；AI Chat 在选择千问时可以通过 `ai-chat-route` 发送最多三张 JPEG/PNG/WebP 图片。Edge Functions 在单次请求内把图片转发给千问多模态能力，校验结构化 Food Draft 或 Workout Draft payload，并且不保存原图或 base64 payload。Add Food 和 AI Chat request logs 会写入通过校验的 `image_count`，chat history 持久化文本 turn，并为返回的 draft 保存轻量 artifact snapshot 和摘要。
...
- 对图片饮食分析，只保存 workflow、model、图片数量、mime type、压缩字节数、schema 校验状态和 safety/error flags 等紧凑 metadata；默认不保存原图或 base64 payload。
