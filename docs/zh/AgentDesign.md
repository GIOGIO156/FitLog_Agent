...
用户请求
  ...
  -> 确定性 workflow 与 expected-output 解析
...
| Add Food AI 分析 | 把文字和零到三张图片发送给 `ai-food-photo-analyze`，校验 Food Draft 并打开 Food Preview。 | 服务端中介的 draft generation。 |
| AI Chat | 所有 gates 通过后，通过 OpenAI/Qwen 发送文字、通过 Qwen 发送图片；可以返回 text、Food Draft 或 Workout Draft。 | 服务端中介的 answer 或 draft generation。 |
...
所有 provider reply 在 Gateway 完成 provider-independent envelope、expected output、领域规则和 write policy 校验前都不可信。OpenAI 使用 strict Structured Outputs；Qwen 使用 JSON Mode 加同一确定性 validator。可纠正结构失败最多一次受限纠错。完整规则见 [AIOutputContract.md](AIOutputContract.md)。
