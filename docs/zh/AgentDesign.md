...
FitLog_Agent 从 FitLog Local 复制而来。当前代码仍然保留本地确定性饮食记录、训练记录、Profile 设置、饮食算法、SQLite 存储和导出能力。Phase 1 已新增居中的 AI tab 和不可用 AI shell；Phase 2 新增账号、订阅状态和 Cloud Profile 基础。AI Gateway、远程模型调用、云端 chat history 和 RAG 仍属于后续阶段。Agent V1 不是自动 AI Coach，也不是完整云同步平台。
...
当前源码还没有 App 内部 LLM 执行能力。Phase 1-2 实现了 AI 导航入口、不可用 Chat shell、账号/订阅状态入口、Profile 登录 gate、Cloud Profile mapper/repository 路径和本机记录摘要授权。
...
| AI Chat shell | 居中的 AI tab，包含不可用背景、可编辑输入框、模型选择器、历史入口占位和账号/订阅状态入口；Phase 3 前不能发送。 | 否 | `AiPage`, `FitLogBottomNavBar` |
...
| 本机记录授权 | 每账号本地设置，控制未来 AI 回答是否可使用本机 food/workout/weight 摘要。Phase 2 不上传完整历史。 | 否 | `AiLocalContextPermissionRepository`, `AiPage` |
...
当用户请求需要时，本地记录可以被汇总后临时发送给 AI Gateway。在明确的 Phase 7 云同步迁移前，本地 food/workout/weight/body-metric 记录是设备数据集，不是账号级云端数据。它们不能静默归属到新登录账号；当账号归属可能有歧义时，AI context builder 应只使用紧凑摘要，并配合用户可见的授权或设置。
...
- Profile UI 修改先进入页面本地草稿。点按和输入会立刻更新 Profile 页预览，已改区块显示更醒目的已修改标记，底部“保存更改”条贴近 Profile body 底部并向上展开紧凑改动摘要，然后一次性 upsert 完整 Cloud Profile snapshot。
- Profile 页面里的当前身体指标，包括体重、体脂和腰围，登录后属于 Cloud Profile snapshot。它们的历史趋势记录在 Phase 2-6 仍保留为本地、按账号作用域的数据。
...
- Profile 页面底部的账号卡片提供明确的退出登录入口。退出登录或切换账号会清空 auth session、运行期 Profile 草稿状态、账号绑定的 Cloud Profile 缓存元数据和本地 singleton Profile 缓存；不得删除设备本地 food、workout 或 weight 历史。
