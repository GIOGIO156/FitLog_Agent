...
FitLog_Agent 从 FitLog Local 复制而来。当前代码仍然是本地确定性饮食记录、训练记录、Profile 设置、饮食算法、SQLite 存储和导出能力。Agent V1 会在这个基础上新增云端 AI 辅助层，但它不是自动 AI Coach，也不是完整云同步平台。
...
当前从 Local 复制来的源码还没有 App 内部 LLM 执行能力。
...
底部导航应改为浮动白色 pill。pill 本身可以是柔和玻璃感或接近不透明的表面，但组件外部不再绘制整行绿色背景。这个规则全局适用，避免 AI 页和彩色背景切换时出现突兀底色带。
