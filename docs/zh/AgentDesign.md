...
| Phase 5 RAG 与 evidence | `ai-chat-route` 为只读用餐决策、周复盘和 App 规则问题做 workflow routing，构建最小必要服务端上下文，检索 `document_chunks`，拦截不支持的写入/隐私请求，返回用户可见 evidence summary，并更新 compact debug summary。 | 服务端中介只读上下文 | `workflow_router.ts`, `context_builders.ts`, `document_rag.ts`, `prompt_builder.ts`, `AiGatewayEvidence` |
...
| 用户记录摘要授权 | 每账号本地设置，控制 Phase 5 用餐决策和复盘 workflow 是否可以使用服务端构建的记录摘要。客户端仍不上传完整业务历史；授权关闭时记录摘要维度会被省略，并作为缺失 evidence 展示。 | 否 | `AiLocalContextPermissionRepository`, `AiPage`, `AiGatewayRequest.allowRecordSummaryContext` |
...
等待 provider 回复时，应由 chat UI 表达活跃状态，而不是让背景加速。发送后 composer 立即清空，用户消息进入 pending 气泡；等这个气泡有了真实布局位置后，消息列表把它锚到顶部操作区下方的可读边界。为了让 pending 用户气泡和 assistant loading 气泡稳定靠上，可以保留很小的活动轮次尾部填充，但它不能表现成一大段可滚动空白，用户手势也不能把 pending/loading 这一组完全滑出视野。assistant loading 气泡显示到最终 assistant 消息从云端 history 重新加载完成。它的进度文案只由客户端保守生成：可以根据请求类型和等待时长说明正在发送、正在等待、图片请求可能更慢、服务端或模型响应较慢，但不能暴露模型真实思考链，也不能在没有 App 或 Gateway 证据时声称已经完成图片识别、营养计算、RAG 检索或摘要读取。assistant 回复出现后不再强制二次滚动。

当同一条用户 turn 同时包含图片附件和文字时，渲染层应把图片预览显示为同一 turn 内右对齐的裸圆角 media，并把文字显示为独立用户气泡。这样可以避免 media 被文字宽度拉伸或产生多余 padding，同时保留一次请求、同一套 pending/重试生命周期和一条云端 history turn。消息缩略图在键盘 inset 重建时应保持稳定解码 bytes 和 gapless image playback，避免键盘弹出/收起期间短暂闪成主题色占位块。
...
- 消息列表 viewport 从 history/account/provider 控件下方开始，而不是屏幕物理顶端。
...
当前 AI Chat 路径实现紧凑同会话 context，并为 routed read-only workflows 实现 Phase 5 受控 RAG。同会话 context 发送近期文本 turn 以及 Food Draft / Workout Draft artifact 摘要，让模型理解当前会话。Phase 5 context 只在 auth、subscription 和 active-device 校验后由服务端构建，不发送历史原图、base64 payload、完整业务历史、用户自由文本 notes 或任意数据库结果。
...
- 只读 workflow 可以解释或建议用户确认的 UI 步骤，但不能保存、删除、修改 Profile、修改目标或应用策略。
...
文档修改不会自动同步到云端 RAG。如果稳定 docs 或 README 的改动会影响 App Logic Q&A，应重新生成 document seed 并在 Supabase 执行。只有 retrieval、prompt、schema 或 evidence 代码变化时，才需要重新部署 Edge Function。当前 seed generator 会在插入新 chunks 前清理已索引来源路径对应的 managed document corpus，避免标题重命名或删除后留下可检索的旧 rows。
...
已实现的食物分析和图片路径保持受限。Add Food 会把文字描述和零到三张压缩后的 JPEG/PNG/WebP 图片发送到 `ai-food-photo-analyze`；AI Chat 在选择千问时可以通过 `ai-chat-route` 发送最多三张 JPEG/PNG/WebP 图片。Phase 5 RAG 可以为只读 workflow 添加服务端构建的摘要和文档片段，但 Edge Functions 仍只转发当前请求输入和受控上下文，校验结构化 Food Draft 或 Workout Draft payload，并且不保存原图或 base64 payload。Add Food 和 AI Chat request logs 会写入通过校验的 `image_count`，纯文字食物分析为 `0`；chat history 持久化文本 turn，并为返回的 draft 保存轻量 artifact/evidence snapshot 和摘要。
...
- Supabase Document RAG schema 与 seed: `supabase/migrations/202607080001_phase5_document_rag_index.sql`, `supabase/seed_phase5_document_chunks.sql`
