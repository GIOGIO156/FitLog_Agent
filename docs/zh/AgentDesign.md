...
FitLog_Agent 从 FitLog Local 复制而来。当前代码仍然保留本地确定性饮食记录、训练记录、Profile 设置、饮食算法、SQLite 存储和导出能力。Phase 1 已新增居中的 AI tab 和不可用 AI shell；Phase 2 新增账号、订阅状态和 Cloud Profile 基础。Phase 3 Cloud Records Foundation 已把登录后的 body/food/workout 正式记录接到云端 source of truth，并新增 Home 选中日期 summary cache 和 stale-while-revalidate；daily summary 云端 upsert coordinator 仍作为 hardening；之后再进入 AI Gateway、远程模型调用、云端 chat history 和 RAG。Agent V1 不是自动 AI Coach，也不是让模型自由读写数据库的平台。
...
- Profile 页面里的当前身体指标，包括体重、体脂和腰围，登录后属于 Cloud Profile snapshot。历史体重、体脂和腰围属于云端 `body_metric_logs`，本地 `user_weight_logs` 只做 confirmed cache。身体资料卡提供日历/新增记录入口，Body Trends 只读展示趋势。
