...
FitLog_Agent 从 FitLog Local 复制而来。当前代码仍然保留本地确定性饮食记录、训练记录、Profile 设置、饮食算法、SQLite 存储和导出能力。Phase 1 已新增居中的 AI tab 和不可用 AI shell；Phase 2 新增账号、订阅状态和 Cloud Profile 基础。Phase 3 应先完成 Cloud Records Foundation，把 body/food/workout 正式记录和 daily summaries 统一到云端 source of truth，再进入 AI Gateway、远程模型调用、云端 chat history 和 RAG。Agent V1 不是自动 AI Coach，也不是让模型自由读写数据库的平台。
...
当前源码还没有 App 内部 LLM 执行能力。Phase 1-2 实现了 AI 导航入口、不可用 Chat shell、账号/订阅状态入口、Profile 登录 gate、Cloud Profile mapper/repository 路径和用户记录摘要授权；Cloud Records、AI Gateway 和 wrapper 尚未实现。
...
V1 云端存储用于账号绑定的正式记录和 AI 体验；本地 SQLite 只是 partial cache、草稿和运行期加速层。
...
当用户请求需要时，云端记录可以被 summary/context builder 汇总后临时发送给 AI Gateway。AI context builder 应只使用紧凑摘要，并配合用户可见的授权或设置；不能把本地 SQLite cache 当作权威上下文。
...
- Profile 页面里的当前身体指标，包括体重、体脂和腰围，登录后属于 Cloud Profile snapshot。历史体重、体脂和腰围在 Phase 3 后属于云端 `body_metric_logs`。身体资料卡提供日历/新增记录入口，Body Trends 只读展示趋势。
