...
- Phase 2 账号/Profile 基础：Supabase 配置入口、邮箱密码与注册验证码 auth repository、订阅状态 repository、内部兑换码 entitlement RPC、Cloud Profile repository/mapper、Profile 登录 gate、Cloud Profile 自动初始化、Profile 订阅卡、AI 账号/订阅状态 sheet、本机记录摘要授权开关。
...
- Phase 2-6 中，food/workout/weight 历史是本机设备数据集，不会静默归属到新登录账号。
- 完整业务记录上云作为 Phase 7 单独设计，不塞进 V1 可用版。
- AI 请求只上传当前任务最小必要上下文。
...
| Phase 7 | 是 | 完整业务记录云同步。 | 账号归属、端云冲突、删除、迁移和隐私边界。 |
...
- 是否认可 Phase 1-6 作为 V1 可用版安装审查阶段，Phase 7 作为后续上云阶段。
- 是否认可 V1 不做完整业务数据云同步。
- 是否认可先只读、再草稿、再确认写入。
...
- 用户可以选择是否允许 AI 后续使用本机记录摘要作为回答依据；默认不允许。

### 为什么现在做

AI Chat、RAG 和 Food Draft 都需要用户身份和 Cloud Profile。先接账号和 Profile，可以避免后面 AI context 依赖一个临时本地 profile，导致之后大面积重构。

Phase 2 不是“把 App 变成云同步 App”。它只是建立账号、订阅、Cloud Profile 和权限状态。完整 food/workout/weight 上云已经拆到 Phase 7。
...
3. 本机记录不自动归属账号。
   - food/workout/weight 仍保存在本地 SQLite。
   - 登录不会上传历史记录。
   - 登出不会删除本机业务记录。
   - AI 使用本机记录摘要前需要用户授权。
...
5. Phase 2 不接模型。
   ...
   - 不发送 AI 消息。
   - 不保存 chat history。
   - ready 状态只是说明账号、网络、订阅条件满足，真正发送在 Phase 3。
...
Phase 2 使用 Supabase Auth + Postgres。Flutter 端可以使用 Supabase client 访问 Auth 和受 RLS 保护的表；AI Gateway 仍留到 Phase 3。
...
```text
status: signedOut|offline|subscriptionInactive|profileMissing|readyForPhase3
canEditComposer: true
canSend: false in Phase 2
reason
```
...
Phase 2 中 `canSend` 建议保持 false，因为没有 AI Gateway。UI 可以进入 ready visual state，但发送按钮应显示 disabled 或 Phase 3 coming-soon 行为，不能触发空请求。
...
| 已登录、在线、已订阅、有 Cloud Profile | ready visual | 可编辑 | disabled 或 Phase 3 placeholder |
...
- 文案要说明“只允许后续 AI 使用本机记录的必要摘要，不上传完整历史”。
...
- 开启“允许 AI 使用本机记录摘要”后状态保存；换账号默认不沿用。
...
## 13. Phase 7: 完整业务记录云同步

### 目标

在 V1 可用版稳定后，再单独设计和实现 food/workout/weight 完整云同步。

Phase 7 不改变 Phase 2-6 的默认规则：在云同步迁移前，本地业务记录是设备数据集，不会静默归属到新登录账号。

### 为什么后置

完整云同步不是简单把 SQLite 表复制到云端。它需要同时解决账号归属、历史迁移确认、端云 source of truth、离线队列、冲突合并、删除账号、导出、隐私说明和一致性测试。如果提前塞进 Phase 2，会和账号/Profile/订阅基础混在一起，导致 V1 可用版无法稳定收敛。
...
| Chat History | 不适用 | 不适用 | 必须 | 必须 | 必须 | 必须 | 回归 |
| RAG | 不适用 | 不适用 | 不适用 | 必须 | 必须 | 必须 | 回归 |
| Food Draft | 不适用 | 不适用 | 不适用 | 不适用 | 必须 | 必须 | 回归 |
| 业务记录云同步 | 不适用 | 不适用 | 不适用 | 不适用 | 不适用 | 不适用 | 必须 |
...
Phase 7 是 V1 可用版之后的业务记录上云阶段，不是 V1 完成的前置条件。
