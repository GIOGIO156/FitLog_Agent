...
当前代码以 FitLog Local 的产品和确定性算法为基线，Agent V1 的账号、云端正式记录、AI Gateway、草稿合同与 Phase 5 RAG 主体已经落地。Phase 5 与后续 Chat orchestration source plans 已退场；当前状态和剩余发布 Gate 由本 Roadmap 的 Phase 7 统一承接，不回退重做 Phase 4/5。历史实施证据见 [Phase 5 AI/RAG 最终工程记录](history/phase5/PHASE5_AI_RAG_FINAL_ENGINEERING_RECORD.md)。
...
- Provider-neutral `chat_decision.v2`、typed/persisted clarification、当前图片生命周期、历史 decision shadow diff、真实 production-path fixture runner 和细分系统错误已经部署；migration、真实 auto canary、受控 soak、回滚演练和 configured split APK 已完成，legacy 生产分支已退场。最新 corpus 激活后的重复生产 recheck 在 typed-clarification answer consumption 上收到 Qwen `provider_incomplete`，因此该消费/幂等重放 Gate 与有效测试设备上的原始故障及附件重启人工 journey 一并保持开放。
...
| 阶段 | 当前状态 | 是否需要安装审查 | 核心目标 | 主要风险 |
| --- | --- | --- | --- | --- |
| Phase 0 | 已落地 | 否 | 技术选型、API contract、工程边界锁定。 | 后续边做边改方向。 |
| Phase 1 | 已落地 | 是 | App Shell 与 AI 页面空壳。 | 导航和 UI 破坏现有 Local 体验。 |
| Phase 2 | 已落地 | 是 | 账号、订阅、Cloud Profile 基础。 | 登录状态和 Profile 权威来源混乱。 |
| Phase 3 | 已落地 | 是 | Cloud Records Foundation：正式记录云端化、本地 partial cache、summary API/hardening。 | 数据源混乱、cache 误当权威、记录删除/分页/summary 出错。 |
| Phase 4 | 已落地 | 是 | AI Gateway 与云端 Chat History。 | 网络、订阅 gating、会话持久化出错。 |
| Phase 5 | 完成并归档 | 是 | Structured RAG / Document RAG、Output Contract、Foundation remediation 与只读 workflows。 | 上下文上传过多、回答越权、文档语言检索错误。 |
| Phase 6 | 主体已实现，真机证据 Gate 开放 | 是 | Reliability Evaluation Lab 与 Chat orchestration：自动化验证 RAG、回答、多轮状态、安全边界和草稿确认。 | 评测绕过生产入口、指标不能解释、行为替换丢失旧能力。 |
| Phase 7 | 当前阶段 | 是 | V1 Release Hardening：关闭有效真机、隐私、弱网和发布审查 Gate。 | 弱网、删除、隐私、性能、边界遗漏。 |
...
原始 Phase 5 controlled Structured/Document RAG、AI Output Contract 和 foundation remediation 已落地并归档。生产保持 `rag_foundation_v1` 和一次有界 Document RAG retry 启用；OpenAI adapter 与 contract tests 保留，但未配置的 OpenAI 不参与当前 RAG，也不是当前 Qwen 发布依赖。检索继续使用 indexed term/FTS/trigram candidates 与 Qwen query embedding 并发，再由 PostgreSQL v3 保持全局 branch ranking、融合最多 30 个最终 candidates，交给 Edge reranker；complete/conflicting coverage、未知 exact identifier 和 unchanged rewrite 不触发无收益 retry。后续 Chat orchestration 整改没有替换或降级这些能力。
...
Foundation 当时的稳定 owning docs 已在用户明确授权后完成 embedding 与 cloud activation；其收口快照 build 为 `99d908c576c844fd3c39d853`、21 sources / 577 chunks，后续更新形成 586-chunk build `a33cf90c1adf71ec7d08113d`。2026-07-20 再次获得明确授权后，本地 build `bbdd397f3d144e4ccea082e8` 只生成 118 个 missing/stale embeddings、复用 495 个 current vectors、淘汰 79 个旧 local records，并以 21 sources / 613 chunks 原子激活；独立回读为 613 cloud rows、0 mismatch。当时 Edge 为 Chat v65、Food photo v34；同配置 activation recheck 33/33，production Edge retrieval p50/p95 为 926/1244 ms。随后 UI stable-doc 更新形成 build `4ebf00df876ce7cc62738c02`：生成 17 个 missing 与 2 个 stale embeddings、复用 596 个 current vectors、淘汰 15 个旧 local records，并以 21 sources / 615 chunks 原子激活；独立回读为 615 cloud rows、0 mismatch。相同制品 SHA 的 Edge 已重部署为 Chat v66、Food photo v35。三次完成的 production canary 均为 30/33；检索保持 13/13 recall、37/39 precision、5/5 critical top-1、零 embedding fallback，最新 Edge retrieval p50/p95 为 968/1236 ms，但 Qwen `provider_incomplete` 使 typed-clarification consumption、replay idempotency 与 resolved-once Gate 保持开放。

- 最终综合工程记录：[`history/phase5/PHASE5_AI_RAG_FINAL_ENGINEERING_RECORD.md`](history/phase5/PHASE5_AI_RAG_FINAL_ENGINEERING_RECORD.md)
...
本轮已实现中文分词、双语术语归一化、稳定文档 embedding、indexed controlled hybrid retrieval、reranker、受控检索工具、最多一次 Agentic retry、前置 Task/Context Planning、动作定义/历史 Context、Workout Draft 确定性绑定、grounded answer guard 和全面评测。其评测资产由 Phase 6 Reliability Evaluation Lab 继续复用；legacy Chat decision 生产分支已经退场，只保留历史版本和行为 oracle，不能成为 silent fallback。
...
Phase 6 的 deterministic eval、Document RAG retrieval、Structured Context、output/safety、behavior parity、multi-turn journey、真实 `auto` cloud canary、受控 soak 和 rollback rehearsal 主体已经随 Foundation 与 Chat orchestration 整改落地。当前 `chat_decision.v2` 是唯一生产决策，typed clarification、附件状态、failure taxonomy 和旧能力 parity 均有自动化与云端证据；不能再把本阶段描述成尚未建立的未来实验室。

最新稳定文档的外部 embeddings、原子激活和 cloud/local parity 已在明确授权后完成。最新 production recheck 的 corpus、检索、权限、Food 和图片链路通过，但 typed-clarification answer consumption 连续收到 Qwen `provider_incomplete`，其消费/幂等重放 Gate 尚未关闭；有效 Android 测试设备上的原始故障及 picker 重建 journeys 也仍待执行。下文保留完整评测范围和门槛，作为已实现资产的回归合同及 Phase 7 验收依据，而不是要求重建另一套 eval 基础设施。
...
Phase 7 是当前阶段。它不回退或重做 Phase 5/6，也不能借发布优化关闭 keyword/vector/hybrid/reranker/coverage/retry、放宽 Output Contract、替换现有记录/草稿/恢复 owner，或恢复已退场的 legacy Chat decision。

最新稳定文档 corpus 的 external embedding、activation、parity 和 Edge redeploy 已完成。Production recheck 的检索质量与 Edge latency Gate 通过，但 typed-clarification answer consumption 连续返回 Qwen `provider_incomplete`，使消费、幂等重放和 resolved-once 三项仍开放；有效用户确认测试设备上的人工 journey 也仍待执行。一般发布监控、隐私和弱网审查继续按本阶段执行。事前 Git checkpoint 未形成是保留在历史中的流程偏差，不能事后伪造，也不再作为一个永远无法完成的伪 Release Gate。OpenAI 未配置且不是当前 Qwen 发布阻断项；未来启用前仍必须完成其独立 live canary、监控与 adapter rollback。
