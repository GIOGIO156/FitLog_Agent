...
Phase 2 中 `canSend` 建议保持 false，因为没有 AI Gateway。UI 可以进入 ready visual state，但发送按钮应显示 disabled 或 Phase 4 coming-soon 行为，不能触发空请求。
...
- 身体资料卡提供日历/新增身体记录入口。
- 点入口后打开身体记录 sheet，顶部只显示具体日期。
- 身体记录 sheet 只编辑三项：体重、体脂、腰围。
