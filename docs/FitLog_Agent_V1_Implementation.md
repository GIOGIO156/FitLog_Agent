...
V1 的核心不是把整个 App 变成 AI Coach，也不是把所有业务数据立刻迁移到云端。V1 的核心是：

```text
FitLog_Agent V1
= 现有 FitLog 本地记录与确定性算法底座
+ 云端账号 / 订阅 / Profile
+ 云端 AI Gateway
+ 居中的全屏 AI Chat 页面
...
- Food / Workout / Weight 历史 V1 不默认全量云同步。
...
5. **云端 Profile + 本地业务记录的边界是否足够清晰**
   - Profile 跟账号走。
   - 正式 food / workout / weight 历史 V1 不默认全量云同步。
   - AI 请求只上传最小必要上下文。
...
| Database | SQLite additive migrations、food/workout/weight/review 等本地表 |
...
- 自动生成长期训练计划。
- 做医疗诊断或治疗建议。
- 把完整 food / workout / weight 历史默认云同步。
- 把用户业务数据做长期 embedding / semantic memory。

## 2.3 “不是 local-first 主叙事”但仍保留本地业务底座
...
但这不等于 V1 要把所有数据都云端化。更准确的表述是：

> V1 使用云端服务承载账号、订阅、Profile 和 AI 能力；同时保留现有本地 food / workout / weight 记录作为 App 的业务记录底座，不在 V1 默认做完整云同步。
...
> App 端保留既有 food / workout / weight 记录与确定性算法；云端负责账号、订阅、Profile、AI Gateway、Chat history 和模型调用；AI 根据云端 Profile 与必要本地摘要生成草稿、建议、复盘或解释；用户确认后才写入正式本地记录。
...
用户轻量编辑 / 追问 / 放弃 / 打开 full editor / 确认保存
        ↓
如确认保存：调用 Local Repository / Service
        ↓
SQLite 正式记录更新
        ↓
Home / Food Log 等页面刷新
...
| Profile 层 | nickname、身体数据、饮食目标、策略设置 | 云端主版本 + 本地缓存 | 跟账号走 |
| AI 服务层 | chat sessions、messages、final answers、request logs | 云端 | 登录后可回看 |
| 本地业务记录层 | food / workout / weight / local reviews | 本地 SQLite | V1 不默认全量云同步 |
| 本地运行时聚合 | DailySummary、strategy result | App 运行时 | 不作为表长期保存 |
...
- `user_weight_logs` 历史体重 V1 不默认全量上云。
...
- 清理本地缓存；
- 本地 food / workout / weight 数据是否同时删除，需要产品设置中单独确认。
...
当 AI 需要分析用户记录时，App 上传最小必要摘要，而不是完整数据库。
...
Read Tool 只能返回摘要，不直接把完整 SQL rows 塞给模型。
...
Context Builder 负责把本地和云端数据变成最小必要上下文。
...
```text
AI 请求可能会将你的文字、图片、Profile 字段和必要的本地摘要发送到 FitLog AI 网关处理。
```

英文建议：

```text
AI requests may send your message, selected images, profile fields, and necessary local summaries to FitLog AI Gateway.
```
...
full local SQLite
full food history cloud copy
full workout history cloud copy
full weight history cloud copy
...
V1 本地仍以现有 SQLite 为业务记录基础。

可规划扩展：

- `food_records.source` 增加：
  - `ai_photo`
  - `ai_chat`
  - `ai_meal_decision`
...
| V1 范围膨胀 | 变成全云同步平台 | V1 不默认同步 food/workout/weight |
...
+ Weekly Review
+ App Logic Q&A
+ Structured RAG over local summaries
