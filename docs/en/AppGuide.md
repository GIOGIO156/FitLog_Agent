...
Paste AI Result keeps its resting layout fixed and non-scrollable. While the JSON editor owns focus and the keyboard is visible, the page gains only the temporary scroll extent needed to move the editor above the keyboard without shrinking the setup card or overflowing the viewport. Closing the keyboard restores the zero scroll offset and locked resting layout.
...
- The auth landing, sign-in, and registration canvas is non-scrollable while the keyboard is closed. With the keyboard open, only the temporary auth scroll range is enabled; the focused email, code, password, or confirmation field is moved above the keyboard. Closing the keyboard restores the original zero offset.
