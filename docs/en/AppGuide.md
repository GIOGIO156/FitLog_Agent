...
- The Body Profile card provides the calendar/add body-record entry. After the user selects a date, the body-record sheet shows only the exact date at the top and edits only weight, body-fat percentage, and waist circumference. Backfilling a past date must not silently update the current Cloud Profile; setting a historical row as current body profile requires explicit confirmation.
