...
| Add Food | Food creation workflow. | AI Food Analysis is first, accepts a text-only description or up to three optional images, and has an independent ChatGPT/Qwen selector; current requests use Qwen, while unconfigured ChatGPT reports unavailable without sending. Manual entry, a reusable external-chat prompt, and external AI JSON paste remain available. | The provider state does not change the shared Food policy, semantic Preview Gate, privacy, or confirmation boundary. |
...
- The AI message list treats the composer as a real obstruction without a full-width footer plate. Its viewport starts below the top action row, hard-clips old messages, and keeps a short bottom fade. With the keyboard closed, the viewport ends above the composer. With the keyboard open, the composer holds a 12 px gap above the keyboard while a bounded blurred gradient veil covers the lower reading edge; the list cannot be manually scrolled, and the first tap outside the composer dismisses the keyboard instead of activating obscured page content. Closing the keyboard restores normal scrolling and the full reading area. Composer motion remains bounded between the keyboard-aware position and normal navigation-resting clearance.
...
- The usable Qwen selection is stored locally on the device with `SharedPreferences` without cloud sync. An unavailable ChatGPT selection is not persisted as the active provider and automatically returns to Qwen after brief tap feedback. The status pill is independent from provider availability and shows account, subscription, Profile, device, network, and Gateway readiness only: it remains `Ready` during an unavailable ChatGPT tap when those gates permit sending, and becomes `Off` only when a readiness gate blocks sending. In-progress sending is shown by the send-button spinner and assistant loading bubble, not by the readiness pill.
...
Add Food exposes AI Food Analysis as the first entry. The user can type a food description by itself, or add up to three optional camera/gallery images, tap thumbnails to switch the enlarged preview, and use a model selector independent from AI Chat. Qwen sends the input to the dedicated `ai-food-photo-analyze` Gateway path; selecting unconfigured ChatGPT shows the same transient unavailable error, slides the selector back to Qwen, retains text and images, and sends nothing. Before launching the system camera/gallery picker, the app stores a small recovery marker so an Android activity restart can reopen the analysis draft instead of dropping the user back to an empty Home state. The configured image-capable server adapter uses the same Food fact priority, target-language policy, semantic validator, correction limit, and no-write boundary. A successful terminal result must contain a validated Food Draft and cannot be replaced by ordinary explanation text; invalid output keeps the form input for revision. The draft opens in the existing Food Preview editor, and only the user's Save action writes `food_records` / `food_items` with source `ai_photo`.
