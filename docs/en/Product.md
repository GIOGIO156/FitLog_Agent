...
- After the app leaves the foreground, Android mirrors an active unsaved new-workout draft with any selected exercise as a system "workout in progress" notification. This is a draft-resume surface, not a background task or new official record. Editing an existing official workout is page-local and never creates this retained draft or notification.
  ...
  - Tapping cancels the notification before resuming exactly one editor for the same local draft. Returning to the foreground, save, discard, or deleting every exercise also cancels it.
...
If Android rebuilds the process while a new-workout editor is active, a local activity hint and the authoritative SQLite draft restore Add Workout automatically only when the draft was updated within the previous 30 minutes. A surviving process keeps the existing route without another push. Older drafts open only through the existing Workout resume bar. Explicit back, discard, and successful save clear the automatic-resume hint; no timer, alarm, foreground service, wake lock, or background keepalive is used. Draft persistence remains critical and ordered. The best-effort workout notification mirrors the latest draft only while the app is outside the foreground; returning to the app or tapping the notification cancels it before one editor route is restored.

Starting an official workout save first persists a stable mutation id, target plan, payload hash, calculation weight, timestamp, and `committing` state, then freezes lifecycle autosave and editing. The cloud writes the workout sessions and sets in one transaction and records the mutation result, so retrying the same mutation cannot create a second official record. Cloud-confirmed completion is followed by final ordered draft deletion.

If the request outcome is ambiguous because the app backgrounds, the process stops, or the connection drops, the retained item is shown as a locked save-confirmation state rather than an editable workout draft. A surviving process queries the same mutation result, and a manual retry reuses the same payload and mutation id. On a new process launch, FitLog atomically confirms or abandons that mutation without resubmitting the workout: a committed result clears the local item, an abandoned result unlocks the ordinary draft, and an unavailable status leaves it locked. The server records abandonment before a late copy of the old request can write an official record. The Android workout-in-progress notification and automatic editor restoration stay disabled while confirmation is pending.
