...
- When the keyboard is closing, intermediate keyboard insets smaller than the closed navigation-resting clearance should be treated as the resting state, so the composer does not travel below the navigation-resting position before settling.
...
8. Official food records are written only after confirmation.
