...
- A workout-plan create or replacement uses `commit_workout_plan_v1`, not a client-side sequence of session/set writes. The RPC accepts one stable `input_mutation_id`, operation (`create`, `replace_plan`, or `replace_session`), target/source identifiers, a SHA-256 `input_payload_hash`, canonical sessions, and active device/session identifiers. It writes the replacement, all sessions/sets, and the mutation result in one transaction.
- `workout_plan_commits` is keyed by authenticated account plus mutation id and stores only operation, target plan, payload hash, status (`pending`, `committed`, or `abandoned`), result session IDs, and timestamps. It does not duplicate workout notes or payload JSON. The same mutation and hash returns the original committed result; a different hash returns `idempotency_conflict`; an abandoned mutation rejects a late copy of the old commit.
- `get_workout_plan_commit_v1(input_mutation_id, input_device_id, input_session_id)` performs read-only foreground confirmation and returns `{"status":"committed", ...}`, `{"status":"abandoned"}`, or `{"status":"not_found"}`. On new-process recovery, `abandon_workout_plan_commit_v1(input_mutation_id, input_operation, input_target_plan_id, input_payload_hash, input_device_id, input_session_id)` shares the commit advisory lock and atomically returns the committed result or writes an abandoned tombstone without resubmitting workout sessions. All three RPCs derive the account from auth, enforce the active-device boundary, and are the only authenticated client access to the commit ledger.
...
- `client_request_id` is a stable per-turn idempotency key generated by Flutter. A retry of the same clarification reply reuses it; a new user action creates a new ID.
- `clarification_reply`, when present, contains `clarification_id`, one allowlisted `option_id`, and the same `client_request_id`. It is valid only for the active account/session clarification and is consumed before ordinary intent inference. Clients cannot submit a result family directly.
...
Current Chat responses use `schema_version = ai_chat_response.v3` and are produced under `prompt_version = chat_orchestration_v2`. These versions are server-owned observability/compatibility facts; Flutter does not choose them.
...
```json
{
  ...
  "clarification": {
    ...
    "clarification_id": "clar_...",
    "kind": "missing_business_fields",
    "question": "这张图里的肉看不清，你知道是鸡肉、牛肉还是猪肉吗？",
    "options": [],
    "missing_dimensions": ["food_identity"],
...
- `provider_unavailable`: selected provider/model is not configured
...
- `planner_unavailable`: bounded planner could not complete
- `planner_output_invalid`: bounded planner returned an invalid decision
- `clarification_conflict`: reply is stale, mismatched, already claimed, or currently resolving
- `clarification_expired`: pending clarification expired
- `attachment_unavailable`: required request-time pixels are no longer available and must be reattached
...
- Non-food or genuinely insufficient combined text/image evidence should trigger a bounded clarification or refusal, not a forced food record. An unclear image alone must not reopen intent selection when clear user text is already sufficient for the selected Food Draft; uncertainty belongs in the draft instead.
