alter table public.document_chunks
  add column if not exists search_tsv tsvector generated always as (
    to_tsvector(
      'simple'::regconfig,
      coalesce(heading, '') || ' ' || coalesce(context_prefix, '') || ' ' ||
        coalesce(content, '')
    )
  ) stored;
...
create or replace function public.search_document_chunks_hybrid_v2(
...
revoke all on function public.search_document_chunks_hybrid_v2(
  text, text[], text, text[], vector, integer, text
) from public, anon, authenticated;
grant execute on function public.search_document_chunks_hybrid_v2(
  text, text[], text, text[], vector, integer, text
) to service_role;
