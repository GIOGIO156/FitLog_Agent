...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const providerContent = await callQwenVisionProvider(
      providerConfig,
      parsedRequest,
    );
    const parsedProvider = parseProviderFoodDraftBody(providerContent);
...
function readQwenVisionConfig(): QwenVisionConfig {
  ...
  const model =
    Deno.env.get("FITLOG_QWEN_VISION_MODEL") ??
  ...
  return {
    ...
    timeoutMs: positiveInt(Deno.env.get("FITLOG_AI_PROVIDER_TIMEOUT_MS"), 30000),
  };
}
...
async function callQwenVisionProvider(
  ...
  try {
    const response = await fetch(config.baseUrl, {
      ...
      body: JSON.stringify(
        buildQwenVisionRequestBody({ request, model: config.model }),
      ),
    });
    if (!response.ok) {
      throw new PhotoGatewayHttpError("provider_failure", 502);
    }
    return extractQwenContent(await response.json());
  } catch (error) {
