...
function deterministicPlan(request: GatewayRequest): TaskPlanV1 | null {
  ...
  if (isWorkoutLog(text)) return plan(request, "workout_logging", "workout_draft", ["exercise_definition"], ["workout_draft_intent"], [], 0.94);
  if (isImplicitFoodLog(text)) return plan(request, "food_logging", "food_draft", [], ["implicit_food_log"], [], 0.9);
  if (isSameChatWorkoutContinuation(request)) return plan(request, "workout_logging", "workout_draft", ["exercise_definition"], ["same_chat_workout_entity"], [], 0.9);
...
function isWorkoutLog(text: string): boolean {
  return /(?:记录|添加|log|record).{0,20}(?:训练|深蹲|分腿蹲|卧推|硬拉|组|reps?|sets?|workout)|(?:训练|深蹲|分腿蹲|卧推|硬拉|workout).{0,30}(?:\d+\s*组|\d+\s*(?:次|reps?))/i.test(text);
}
...
function isSameChatWorkoutContinuation(request: GatewayRequest): boolean {
  if (!/(?:它|这个|再|加一组|add another set|one more set)/i.test(request.messageText)) return false;
  return request.conversationContext?.artifacts.at(-1)?.type === "workout_draft" || request.conversationContext?.messages.slice(-2).some((message) => /训练|workout|exercise/i.test(message.text)) === true;
}
